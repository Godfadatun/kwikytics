<template>
  <div class="flex flex-center">
    <q-card class="" style="min-width: 300px; max-width: 800px">
      ...
      <q-separator />
      <q-card-section class="no-padding gt-sm">
        <q-card class="row ">
          <q-card square class="no-padding full-height col-md-4 " style="min-height: 300px; max-height: 300px; ">
          ...
          <q-card square class="col-md-8 " style="border-left: solid grey 1px; min-width: 300px" >
      ...
      <q-card-section class="lt-md no-padding">
        <q-card flat v-if="contact == true" square class="no-padding full-height" style="min-height: 300px; max-height: 300px; ">
          <q-toolbar class="bg-grey-4">
            <div class="text-bold text-primary">Contacts</div>
          </q-toolbar>
          <q-list separator bordered>
            <q-item v-for="(conversation, index) in conversations" :key="conversation.id" clickable v-ripple @click="()=>{currentConversationIndex = index; contact = false}" >
              <q-item-section avatar>
                <q-avatar  color="primary" text-color="white" :icon="conversation.avatar" />
              </q-item-section>
        ...
        <q-card flat v-if="contact == false" square class="" style="border-left: solid grey 1px; min-width: 300px" >
          <q-toolbar class="bg-grey-3 text-black">
            <q-btn round flat icon="keyboard_arrow_left" @click="contact = true" />
            <q-btn round flat>
              <q-avatar color="primary" text-color="white">
                <q-icon :name="currentConversation.avatar" size="sm" />
              </q-avatar>
            </q-btn>
            <span class="q-subtitle-1 q-pl-md"> {{ currentConversation.person }} </span>
          </q-toolbar>
